...
function doGet(e) {
 ...
 const tokenVip = e.parameter.token || ''; // Tangkap token VIP dari URL

 // Jika bukan email UII DAN tidak punya token VIP, arahkan ke error auth
 if (!isUiiEmail && !tokenVip) {
 ...
 template.tokenVip = tokenVip; // Titipkan token VIP ke HTML
 return template.evaluate().setTitle('Metive 4.0').setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL).addMetaTag('viewport', 'width=device-width, initial-scale=1, maximum-scale=1.0, user-scalable=no, viewport-fit=cover');
}
...
function getInitialData(tokenVip = '') {
 try {
   ...
   const profile = getProfile(sheetUsers, tokenVip);
...
function addCalendarEvent(payload) {
    const lock = LockService.getScriptLock(); if (!lock.tryLock(5000)) throw new Error('Sistem sibuk.');
    try {
        ...
        const profile = getProfile(sheetUsers, payload.tokenVip); if (profile.nama === 'Guest') throw new Error('Akses ditolak.');
...
function editCalendarEvent(payload) {
    ...
    try {
        const ss = SpreadsheetApp.openById(SPREADSHEET_ID); const sheetUsers = ss.getSheetByName(SHEET_USERS); const sheetCal = ss.getSheetByName(SHEET_CALENDAR);
        if (!sheetCal) throw new Error('Sheet CalendarEvents belum dibuat.');
        const profile = getProfile(sheetUsers, payload.tokenVip); const data = sheetCal.getDataRange().getValues(); let updated = false;
...
function deleteCalendarEvent(payload) {
    ...
    try {
        const ss = SpreadsheetApp.openById(SPREADSHEET_ID); const sheetUsers = ss.getSheetByName(SHEET_USERS); const sheetCal = ss.getSheetByName(SHEET_CALENDAR);
        if (!sheetCal) throw new Error('Sheet CalendarEvents belum dibuat.');
        const profile = getProfile(sheetUsers, payload.tokenVip); const data = sheetCal.getDataRange().getValues(); let deleted = false;
        for (let i = data.length - 1; i >= 1; i--) {
            if (String(data[i][0]) === String(payload.id)) {
...
// ================== PRESENSI LAMA ==================
function getProfile(sheetObj, tokenVip = '') {
 try {
   const email = getUserEmail_().toLowerCase().trim(); 
   let sh = sheetObj; if (!sh) { const ss = SpreadsheetApp.openById(SPREADSHEET_ID); sh = ss.getSheetByName(SHEET_USERS); }
   const def = { email: email || 'guest@vip', nama:'Guest', nim:'-', divisi:'-', foto_fileid:'', week: '' }; if (!sh) return def;
   ...
   const idxDiv = header.indexOf('divisi'); 
   const idxFoto = header.indexOf('foto_fileid'); 
   const idxWeek = header.findIndex(h => h === 'week' || h === 'kelompok' || h === 'jadwal');
   
   // Deteksi Kolom Token (Fallback ke index 6 / Kolom G jika header 'token' tidak ketemu)
   let idxToken = header.indexOf('token');
   if (idxToken === -1) idxToken = 6; 

   if (idxEmail === -1) return def;
   
   const userRow = values.slice(1).find(row => {
     const rowEmail = String(row[idxEmail] || '').toLowerCase().trim();
     const rowToken = String(row[idxToken] || '').trim();
     if (email && rowEmail === email) return true; // Login biasa
     if (tokenVip && rowToken === tokenVip) return true; // Login VIP
     return false;
   });

   if (userRow) { 
     return { 
       email: String(userRow[idxEmail] || '').toLowerCase().trim() || 'guest@vip', 
       nama: (idxNama !== -1 ? String(userRow[idxNama] || '-') : '-'), 
       nim: (idxNim !== -1 ? String(userRow[idxNim] || '-') : '-'), 
       divisi:(idxDiv !== -1 ? String(userRow[idxDiv] || '-') : '-'), 
       foto_fileid: (idxFoto !== -1 ? String(userRow[idxFoto] || '').trim() : ''), 
       week: (idxWeek !== -1 ? String(userRow[idxWeek] || '').trim() : '') 
     }; 
   } 
   return def;
...
function submitAttendance(payload) {
 const lock = LockService.getScriptLock(); if (!lock.tryLock(5000)) { throw new Error('Sistem sedang memproses presensi. Silakan tunggu beberapa saat.'); }
 try {
   const ss = SpreadsheetApp.openById(SPREADSHEET_ID); const sheetUsers = ss.getSheetByName(SHEET_USERS); const sheetLog = ss.getSheetByName(SHEET_LOG); const profile = getProfile(sheetUsers, payload.tokenVip);
